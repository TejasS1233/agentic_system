...
class MetricsCollector:
    ...
    def end_task(
        self, completed: bool = True, output_correct: Optional[bool] = None
    ) -> TaskMetrics:
    ...
    def record_dispatcher_routing(
        self, zone: str, routing_time_ms: float, expected_zone: Optional[str] = None
    ):
    ...
    def start_turn(self) -> int:
        ...
        if self.current_task:
            ...
            turn = TurnMetrics(
                turn_number=turn_num, timestamp=datetime.now().isoformat()
            )
    ...
    def end_turn(
        ...
        if self.current_task and self.current_task.turns:
            ...
            if model in self.PRICING:
                ...
                turn.tokens.cost_usd = (input_tokens / 1_000_000) * pricing["input"] + (
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        planning_times = [
            t.planning_time_ms for t in self.all_tasks if t.planning_time_ms > 0
        ]
        execution_times = [
            t.execution_time_ms for t in self.all_tasks if t.execution_time_ms > 0
        ]
        routing_times = [
            t.dispatcher_routing_time_ms
            for t in self.all_tasks
            if t.dispatcher_routing_time_ms > 0
        ]
        ttfts = [
        ...
        summary["performance_metrics"] = {
            ...
            "tasks_per_minute": len(self.all_tasks) / (sum(durations) / 60000)
            if sum(durations) > 0
            else 0,
        }
        ...
        summary["llm_metrics"] = {
            ...
            "average_tokens_per_task": (total_input + total_output)
            / len(self.all_tasks),
            ...
            "max_turns_reached_count": sum(
                1 for t in self.all_tasks if t.max_turns_reached
            ),
            "max_turns_reached_percent": (
                sum(1 for t in self.all_tasks if t.max_turns_reached)
                / len(self.all_tasks)
            )
            * 100,
        }
        ...
        summary["tool_metrics"] = {
            ...
            "average_tool_calls_per_task": len(all_tool_calls) / len(self.all_tasks)
            if self.all_tasks
            else 0,
            ...
            "tool_success_rates": {
                k: {
                    "success_rate_percent": (v["success"] / (v["success"] + v["fail"]))
                    * 100,
            ...
            "overall_tool_success_rate_percent": (
                sum(1 for tc in all_tool_calls if tc.success)
                / len(all_tool_calls)
                * 100
            )
            if all_tool_calls
        ...
        summary["routing_metrics"] = {
            ...
            "public_zone_percent": (len(public_tasks) / len(self.all_tasks)) * 100
            if self.all_tasks
            else 0,
            "private_zone_percent": (len(private_tasks) / len(self.all_tasks)) * 100
            if self.all_tasks
            else 0,
            "routing_accuracy_percent": (
                len(correct_routings)
                / (len(correct_routings) + len(incorrect_routings))
                * 100
            )
            if (correct_routings or incorrect_routings)
            else None,
        ...
        peak_cpus = [
            t.peak_cpu_percent for t in self.all_tasks if t.peak_cpu_percent > 0
        ]
        avg_cpus = [
            t.average_cpu_percent for t in self.all_tasks if t.average_cpu_percent > 0
        ]
        ...
        summary["self_correction_metrics"] = {
            ...
            "error_recovery_rate_percent": (
                sum(errors_recovered)
                / (sum(errors_recovered) + sum(len(t.errors) for t in self.all_tasks))
                * 100
            )
            if sum(errors_recovered) + sum(len(t.errors) for t in self.all_tasks) > 0
            else 0,
            "total_code_regenerations": sum(code_regens),
            "total_commands_reexecuted": sum(cmd_reexecs),
            "total_backoff_time_ms": sum(backoff_times),
            "average_backoff_time_per_retry_ms": (sum(backoff_times) / sum(retries))
            if sum(retries) > 0
            else 0,
        }
        ...
        summary["quality_metrics"] = {
            "task_completion_rate_percent": (completed / len(self.all_tasks)) * 100
            if self.all_tasks
            else 0,
            "tasks_completed": completed,
            "tasks_incomplete": len(self.all_tasks) - completed,
            "output_correctness_rate_percent": (correct / (correct + incorrect) * 100)
            if (correct + incorrect) > 0
            else None,
            "correct_outputs": correct,
            "incorrect_outputs": incorrect,
            "unvalidated_outputs": len(self.all_tasks) - correct - incorrect,
            "execution_success_rate_percent": (exec_success / len(self.all_tasks)) * 100
            if self.all_tasks
            else 0,
            "total_hallucinations_detected": hallucinations,
            "average_hallucinations_per_task": hallucinations / len(self.all_tasks)
            if self.all_tasks
            else 0,
        }
        ...
        for model in models:
            ...
            summary["per_model_breakdown"][model] = {
                ...
                "average_duration_ms": statistics.mean(
                    [t.total_duration_ms for t in model_tasks]
                ),
                "total_tokens": sum(t.total_tokens.total_tokens for t in model_tasks),
                "total_cost_usd": round(
                    sum(t.total_tokens.cost_usd for t in model_tasks), 6
                ),
                "completion_rate_percent": (
                    sum(1 for t in model_tasks if t.task_completed)
                    / len(model_tasks)
                    * 100
                ),
            }
        ...
        for zone in ["public", "private"]:
            ...
            if zone_tasks:
                summary["per_zone_breakdown"][zone] = {
                    ...
                    "average_duration_ms": statistics.mean(
                        [t.total_duration_ms for t in zone_tasks]
                    ),
                    "total_tokens": sum(
                        t.total_tokens.total_tokens for t in zone_tasks
                    ),
                    "completion_rate_percent": (
                        sum(1 for t in zone_tasks if t.task_completed)
                        / len(zone_tasks)
                        * 100
                    ),
                }
    ...
    def save_results(self, filename_prefix: str = "benchmark"):
        ...
        tasks_file = os.path.join(
            self.output_dir, f"{filename_prefix}_tasks_{timestamp}.json"
        )
        ...
        summary_file = os.path.join(
            self.output_dir, f"{filename_prefix}_summary_{timestamp}.json"
        )
    ...
    def print_summary(self):
        ...
        print(
            f"   Total Duration (median): {perf['total_duration']['median_ms']:.2f} ms"
        )
        print(f"   Planning Time (mean): {perf['planning_time']['mean_ms']:.2f} ms")
        print(f"   Execution Time (mean): {perf['execution_time']['mean_ms']:.2f} ms")
        print(
            f"   Dispatcher Routing (mean): {perf['dispatcher_routing_time']['mean_ms']:.2f} ms"
        )
        ...
        print(
            f"   Max Turns Reached: {llm['max_turns_reached_count']} ({llm['max_turns_reached_percent']:.1f}%)"
        )
        ...
        print(
            f"   Overall Success Rate: {tools['overall_tool_success_rate_percent']:.1f}%"
        )
        ...
        print(
            f"   Public Zone: {routing['public_zone_tasks']} ({routing['public_zone_percent']:.1f}%)"
        )
        print(
            f"   Private Zone: {routing['private_zone_tasks']} ({routing['private_zone_percent']:.1f}%)"
        )
        ...
        print(
            f"   Execution Success Rate: {qual['execution_success_rate_percent']:.1f}%"
        )
        if qual["output_correctness_rate_percent"] is not None:
            print(
                f"   Output Correctness: {qual['output_correctness_rate_percent']:.1f}%"
            )
