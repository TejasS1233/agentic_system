...
class BenchmarkVisualizer:
    ...
    def plot_performance_breakdown(self):
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / "01_performance_breakdown.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_token_usage(self):
        ...
        labels = [f'Input\n({llm["total_input_tokens"]:,})', f'Output\n({llm["total_output_tokens"]:,})']
        colors = ['#3498db', '#e74c3c']
        
        ax1.pie(tokens, labels=labels, colors=colors, autopct='%1.1f%%', 
        ...
        if self.summary.get("per_model_breakdown"):
            ...
            bars1 = ax2.bar([i - width/2 for i in x], [t/1000 for t in model_tokens], 
                           width, label='Tokens (K)', color='#3498db')
            
            ax2_twin = ax2.twinx()
            bars2 = ax2_twin.bar([i + width/2 for i in x], [c*1000 for c in model_costs], 
                                width, label='Cost (m$)', color='#2ecc71')
            
            ax2.set_ylabel("Tokens (thousands)", fontsize=10)
            ax2_twin.set_ylabel("Cost (milli-dollars)", fontsize=10)
            ax2.set_xticks(x)
            ax2.set_xticklabels([m.split('/')[-1] for m in model_names], rotation=15, ha='right')
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / "02_token_usage.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_tool_distribution(self):
        ...
        if tool_dist:
            ...
            bars = ax1.bar(tool_dist.keys(), tool_dist.values(), color=colors, edgecolor='black')
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / "03_tool_distribution.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_routing_breakdown(self):
        ...
        wedges, texts, autotexts = ax.pie(
            counts, labels=zones, colors=colors, autopct='%1.0f%%',
        ...
        if accuracy is not None:
            ax.annotate(f'Routing Accuracy: {accuracy:.0f}%', 
                       xy=(0.5, -0.1), xycoords='axes fraction',
                       ha='center', fontsize=12, fontweight='bold',
                       bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
        
        ax.set_title("Privacy Zone Routing Distribution", fontsize=14, fontweight='bold')
        
        plt.tight_layout()
        plt.savefig(self.output_dir / "04_routing_breakdown.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_resource_usage(self):
        ...
        ax1.axhline(y=1024, color='red', linestyle='--', alpha=0.5, label='1GB Reference')
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / "05_resource_usage.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_quality_metrics(self):
        ...
        metrics = {
            ...
            "Tool\nSuccess": self.summary["tool_metrics"]["overall_tool_success_rate_percent"],
            "Routing\nAccuracy": self.summary["routing_metrics"].get("routing_accuracy_percent", 0) or 0
        }
        
        colors = ['#2ecc71' if v >= 80 else '#f39c12' if v >= 50 else '#e74c3c' for v in metrics.values()]
        bars = ax.bar(metrics.keys(), metrics.values(), color=colors, edgecolor='black', linewidth=1.2)
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / "06_quality_metrics.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_per_task_comparison(self):
        ...
        # Duration per task
        ax = axes[0, 0]
        bars = ax.bar(range(len(task_ids)), durations, color=zone_colors, edgecolor='black')
        ax.set_xticks(range(len(task_ids)))
        ax.set_xticklabels([t.split('_')[0] + '_' + t.split('_')[1] for t in task_ids], rotation=45, ha='right')
        ax.set_ylabel("Duration (seconds)")
        ax.set_title("Duration per Task")
        
        # Turns per task
        ax = axes[0, 1]
        bars = ax.bar(range(len(task_ids)), turns, color=zone_colors, edgecolor='black')
        ax.set_xticks(range(len(task_ids)))
        ax.set_xticklabels([t.split('_')[0] + '_' + t.split('_')[1] for t in task_ids], rotation=45, ha='right')
        ...
        # Tool calls per task
        ax = axes[1, 0]
        bars = ax.bar(range(len(task_ids)), tool_calls, color=zone_colors, edgecolor='black')
        ax.set_xticks(range(len(task_ids)))
        ax.set_xticklabels([t.split('_')[0] + '_' + t.split('_')[1] for t in task_ids], rotation=45, ha='right')
        ...
        # Zone distribution
        ax = axes[1, 1]
        zone_counts = {"public": zones.count("public"), "private": zones.count("private")}
        ax.pie(zone_counts.values(), labels=zone_counts.keys(), colors=['#3498db', '#2ecc71'],
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / "07_per_task_comparison.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_turns_distribution(self):
        ...
        ax.axvline(x=sum(turns)/len(turns), color='red', linestyle='--', 
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / "08_turns_distribution.png", dpi=300, bbox_inches='tight')
        plt.close()
            ...
            if tools.get("tool_call_distribution"):
                ...
                for tool, count in tools["tool_call_distribution"].items():
                    ...
                    tool_data.append({
                        ...
                        "Success Rate (%)": round(success.get("success_rate_percent", 0), 1)
                    })
                
                pd.DataFrame(tool_data).to_excel(writer, sheet_name='Tools', index=False)
...
class BaselineComparison:
    ...
    def __init__(self, results_dir: str = "benchmark_results"):
        ...
        for mode in self.modes:
            ...
            if mode_dir.exists():
                try:
                    summary_files = sorted(mode_dir.glob("benchmark_summary_*.json"), reverse=True)
                    if summary_files:
    ...
    def generate_comparison(self):
        ...
        if len(self.mode_results) < 2:
            print(f"⚠️  Need at least 2 modes to compare. Found: {list(self.mode_results.keys())}")
            return
        
        print(f"Generating comparison charts for modes: {list(self.mode_results.keys())}...")
    ...
    def plot_duration_comparison(self):
        ...
        for mode, data in self.mode_results.items():
            ...
            durations.append(data["performance_metrics"]["total_duration"]["mean_ms"] / 1000)
        ...
        ax.set_title("Duration Comparison Across Baselines", fontsize=14, fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / "comparison_01_duration.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_cost_comparison(self):
        ...
        for mode, data in self.mode_results.items():
            modes.append(mode.upper())
            costs.append(data["llm_metrics"]["total_cost_usd"] * 1000)  # Convert to milli-dollars
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / "comparison_02_cost.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_turns_comparison(self):
        ...
        ax.axhline(y=10, color='red', linestyle='--', alpha=0.5, label='Max Turns Limit')
        ...
        ax.legend()
        plt.tight_layout()
        plt.savefig(self.output_dir / "comparison_03_turns.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_quality_comparison(self):
        ...
        metrics = ["Task Completion", "Tool Success"]
        ...
        for mode in modes:
            ...
            completion_rates.append(data["quality_metrics"]["task_completion_rate_percent"])
            tool_rates.append(data["tool_metrics"]["overall_tool_success_rate_percent"])
        
        bars1 = ax.bar([i - width/2 for i in x], completion_rates, width, 
                      label='Task Completion %', color='#2ecc71', edgecolor='black')
        bars2 = ax.bar([i + width/2 for i in x], tool_rates, width,
                      label='Tool Success %', color='#3498db', edgecolor='black')
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / "comparison_04_quality.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def export_comparison_table(self):
        ...
        for mode, data in self.mode_results.items():
            ...
            comparison_data.append({
                ...
                "Task Completion %": round(quality["task_completion_rate_percent"], 1),
                "Tool Success %": round(tools["overall_tool_success_rate_percent"], 1),
            })
...
def main():
    ...
    parser.add_argument("--results-dir", "-r", default="benchmark_results",
