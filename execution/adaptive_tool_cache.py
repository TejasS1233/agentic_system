...
class ToolMetrics:
    ...
    semantic_group: Optional[str] = (
        None  # e.g., "file_ops", "network", "data_processing"
    )
    ...
    def calculate_decay_score(
        self, k: float = 60.0, registry_data: dict = None
    ) -> float:
        """
...
class AdaptiveToolCache:
    ...
    def __init__(
        ...
        self.archive_ttl = (
            archive_ttl  # Time before archived tools are permanently deleted
        )
    ...
    def register_tool(
        ...
        with self._lock:
            ...
            print(
                f"[ATC] Registered: '{name}' (tier={metrics.tier.value}, group={metrics.semantic_group})"
            )
            return metrics
    ...
    def record_usage(
        self, name: str, execution_time_ms: float = 0.0
    ...
    def _remove_tool(self, name: str, reason: str = "unknown") -> Optional[Any]:
        ...
        print(
            f"[ATC] Archived: '{name}' (reason={reason}, score={self._get_decay_score_with_registry(metrics):.4f})"
        )
    ...
    def _archive_tool(
        self, name: str, tool: Any, metrics: Optional[ToolMetrics], reason: str
    ):
    ...
    def _delete_oldest_archived(self):
        ...
        # Find oldest
        oldest_name = min(
            self._archive.keys(), key=lambda n: self._archive[n]["archived_at"]
        )
        ...
        with self._lock:
            ...
            for name, entry in self._archive.items():
                ...
                result[name] = {
                    ...
                    "reason": entry["reason"],
                    "will_delete_at": datetime.fromtimestamp(
                        archived_at + self.archive_ttl
                    ).isoformat(),
        ...
        def cleanup_loop():
            ...
            while not self._stop_cleanup.wait(timeout=interval):
                ...
                with self._lock:
                    ...
                    if expired:
                        print(
                            f"[ATC] Background cleanup: {len(expired)} tools archived due to TTL"
                        )
                    if deleted:
                        print(
                            f"[ATC] Background cleanup: {len(deleted)} tools permanently deleted"
                        )

        self._cleanup_thread = threading.Thread(target=cleanup_loop, daemon=True)
        self._cleanup_thread.start()
        print(
            f"[ATC] Started background cleanup (interval={interval}s, archive_ttl={self.archive_ttl}s, verbose={verbose})"
        )
