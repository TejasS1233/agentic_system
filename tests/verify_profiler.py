...
def test_profiling_modes():
    ...
    for mode in modes:
        ...
        print(
            f"  ✅ {mode.value:12s}: time={profile.execution_time_ms:.3f}ms, "
...
def test_decay_integration():
    ...
    # Create components
    profiler = Profiler(mode=ProfilingMode.LIGHTWEIGHT)
    decay_manager = create_decay_manager(decay_minutes=5.0, auto_cleanup=False)
    ...
    decay_manager.record_usage(
        "calculator", execution_time_ms=profile.execution_time_ms
    )
