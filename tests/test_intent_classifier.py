...
class TestDomainValidation:
    ...
    def test_case_insensitive(self):
        ...
        for input_domain, expected in test_cases:
            ...
            assert result == expected, (
                f"Case insensitive match failed for {input_domain}"
            )
            assert valid == True
    ...
    def test_fuzzy_matching(self):
        ...
        for input_domain, expected in fuzzy_tests:
            result, valid = validate_domain(input_domain)
            assert result == expected, (
                f"Fuzzy match failed: {input_domain} should be {expected}"
            )
    ...
    def test_invalid_domain_fallback(self):
        ...
        for invalid in invalid_tests:
            ...
            assert result == "system", (
                f"Invalid domain {invalid} should fallback to 'system'"
            )
            assert valid == False
...
class TestIntentClassifier:
    ...
    def _run_tests(self, test_cases, category_name):
        ...
        if failed:
            fail_msg = "\n".join(
                [f"  '{q}': expected {e}, got {g} (via {m})" for q, e, g, m in failed]
            )
            assert False, (
                f"{category_name}: {passed}/{len(test_cases)} passed\nFailed:\n{fail_msg}"
            )
