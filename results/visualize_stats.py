...
for m in task_pattern.finditer(content):
    ...
    # Extract perf grades
    grades = (
        re.findall(r"\((\w+)\)", m.group(0)[m.group(0).find("Perf:") :])
        if "Perf:" in m.group(0)
        else []
    )
...
removed = {
    "Chess Gender Ratio Plot",
    "Census Population Plot",
    "Twitter Trends",
    "Cat Image",
    "Brawl Stars Brawler Stats",
    "Brawl Stars Pie Chart",
}
...
fig.suptitle(
    "IASCIS - Task Performance Dashboard",
    fontsize=28,
    fontweight="bold",
    color=TEXT,
    y=0.98,
...
fig.text(
    0.5,
    0.955,
    f"{len(tasks)} Successful Tasks | Avg: {np.mean([t['time_s'] for t in tasks]):.1f}s | "
...
gs = gridspec.GridSpec(
    3, 3, hspace=0.35, wspace=0.3, left=0.06, right=0.97, top=0.93, bottom=0.04
)
...
colors = [
    GREEN if t < 8 else YELLOW if t < 15 else ORANGE if t < 25 else RED for t in times
]

bars = ax1.barh(
    range(len(names)), times, color=colors, height=0.7, edgecolor="none", alpha=0.9
)
ax1.set_yticks(range(len(names)))
ax1.set_yticklabels(names, fontsize=7, color=TEXT)
ax1.set_xlabel("Time (seconds)", color=TEXT, fontsize=10)
ax1.set_title(
    "Task Execution Times", color=TEXT, fontsize=14, fontweight="bold", pad=10
)
...
bars3 = ax3.barh(
    range(len(tool_names)), tool_vals, color=ACCENT, height=0.65, alpha=0.85
)
...
ax3.set_title(
    "Most Used Tools (Top 12)", color=TEXT, fontsize=14, fontweight="bold", pad=10
)
...
n, bin_edges, patches = ax4.hist(
    all_times, bins=bins, color=ACCENT, edgecolor=BG, alpha=0.85, rwidth=0.85
)
...
ax4.set_title(
    "Execution Time Distribution", color=TEXT, fontsize=14, fontweight="bold", pad=10
)
...
ax5.set_title(
    "Steps vs Execution Time", color=TEXT, fontsize=14, fontweight="bold", pad=10
)
...
for t in tasks:
    for cat, keywords in categories.items():
        if any(
            kw.lower() in t["name"].lower() or kw.lower() in t["query"].lower()
            for kw in keywords
        ):
            cat_counts[cat] += 1
...
bars6 = ax6.bar(
    range(len(cat_names)), cat_vals, color=cat_colors, alpha=0.85, edgecolor="none"
)
...
colors7 = [
    GREEN if t < 8 else YELLOW if t < 15 else ORANGE if t < 25 else RED for t in y_times
]

ax7.bar(x_pos, y_times, color=colors7, alpha=0.85, width=0.8, edgecolor="none")
ax7.set_xticks(list(x_pos)[::2])
ax7.set_xticklabels(
    [chrono[i]["name"][:18] for i in range(0, len(chrono), 2)],
    fontsize=6.5,
    color=TEXT,
    rotation=45,
    ha="right",
...
ax7.set_title(
    "All Tasks Timeline (Chronological)",
    color=TEXT,
    fontsize=14,
    fontweight="bold",
    pad=10,
...
if len(y_times) >= 3:
    window = 3
    rolling_avg = [
        np.mean(y_times[max(0, i - window + 1) : i + 1]) for i in range(len(y_times))
    ]
    ax7.plot(
        x_pos,
        rolling_avg,
        ...
        linewidth=2,
        alpha=0.8,
        label=f"{window}-task rolling avg",
    )
    ax7.legend(
        loc="upper right",
        fontsize=9,
