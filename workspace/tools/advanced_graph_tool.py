...
class AdvancedGraphTool:
        ...
        if isinstance(data, dict):
            ...
            if "dates" in data:
                ...
                series_keys = [
                    ...
                    and data[k]
                    and isinstance(data[k][0], (int, float))
                ]
                if dates and series_keys:
                    result = {"x": dates}
                    for i, key in enumerate(series_keys):
            ...
            if "data" in data and isinstance(data["data"], dict):
                ...
                if (
                    first_key
                    and isinstance(nested[first_key], dict)
                    and "dates" in nested[first_key]
                ):
            ...
            if "repos" in data and isinstance(data["repos"], list):
                ...
                return {
                    "labels": [
                        r.get("name", r.get("full_name", "unknown")) for r in repos
                    ],
                    "values": [
                        r.get("stars", r.get("stargazers_count", 0)) for r in repos
                    ],
                }
            ...
            for key in data:
                if isinstance(data[key], list) and data[key]:
                    ...
                    if isinstance(items[0], dict):
                        ...
                        value_key = next(
                            (f for f in value_fields if f in items[0]), None
                        )

                        if name_key and value_key:
                            return {
                                "labels": [
                                    str(item.get(name_key, "")) for item in items
                                ],
        ...
        try:
            ...
            if not output_path:
                ...
                output_path = os.path.join(
                    self.output_dir, f"chart_{chart_type}_{timestamp}.png"
                )
    ...
    def _create_bar_chart(self, ax, data: Dict, colors: List[str], show_values: bool):
        ...
        bars = ax.bar(
            x, values, color=colors[: len(values)], edgecolor="white", linewidth=0.7
        )
        ax.set_xticks(x)
    ...
    def _create_horizontal_bar(
        self, ax, data: Dict, colors: List[str], show_values: bool
    ):
        ...
        bars = ax.barh(
            y, values, color=colors[: len(values)], edgecolor="white", linewidth=0.7
        )
        ax.set_yticks(y)
    ...
    def _create_line_chart(
        self, ax, data: Dict, colors: List[str], legend_labels: Optional[List[str]]
    ):
    ...
    def _create_scatter_plot(self, ax, data: Dict, colors: List[str]):
        ...
        ax.scatter(
            x, y, c=colors[0], s=sizes, alpha=0.7, edgecolors="white", linewidth=0.5
        )
...
if __name__ == "__main__":
    ...
    result = tool.run(
        chart_type="bar",
        data={
            "labels": ["Python", "JavaScript", "Go", "Rust", "Java"],
            "values": [45, 35, 20, 15, 30],
        },
