...
class GitHubTrendingArgs(BaseModel):
    ...
    language: Optional[str] = Field(
        None, description="Filter by programming language (e.g., 'python', 'javascript', 'go')"
    )
...
class GitHubTrendingTool:
    ...
    def run(
        ...
        try:
            if trending_type == "repos":
                ...
                return {"error": f"Unknown trending_type: {trending_type}. Use 'repos' or 'developers'"}
    ...
    def _fetch_page(self, url: str) -> BeautifulSoup:
        ...
        try:
            ...
            raise ConnectionError(f"Cannot connect to {url}. DNS/network issue? Error: {e}")
    ...
    def _get_trending_repos(
        ...
        if spoken_language and spoken_language.lower() not in ("all", "any", "none", "en", ""):
