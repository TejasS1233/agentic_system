...
class ImageProcessorToolArgs(BaseModel):
    ...
    operation: str = Field(..., description="Operation: 'resize', 'crop', 'rotate', 'filter', 'grayscale', 'convert', 'adjust'")
    params: Optional[str] = Field(None, description="JSON string or dictionary of parameters for the operation (e.g. {'width': 100, 'height': 100} for resize)")
...
class ImageProcessorTool:
    ...
    def run(self, image_path: str, operation: str, params: Union[str, Dict] = None, output_path: str = None) -> str:
        ...
        try:
            ...
            if op_lower in ["grayscale", "greyscale", "bw", "black_and_white", "blackandwhite", "monochrome"] or ("black" in op_lower and "white" in op_lower):
                ...
                if brightness is not None:
                    processed = ImageEnhance.Brightness(processed).enhance(float(brightness))
                if contrast is not None:
                    processed = ImageEnhance.Contrast(processed).enhance(float(contrast))
                if sharpness is not None:
                    processed = ImageEnhance.Sharpness(processed).enhance(float(sharpness))
            ...
            if not output_path:
                ...
                if container_output.exists() and container_output.is_dir():
                    output_path = str(container_output / f"{input_path.stem}_{operation}{suffix}")
                else:
                    # Fallback to same directory (local execution)
                    output_path = str(input_path.with_stem(f"{input_path.stem}_{operation}"))
