...
class SerpSearchArgs(BaseModel):
    ...
    search_type: Literal["search", "images", "news", "shopping"] = Field(
        "search", description="Type of Google search. MUST be one of: 'search' (for web results), 'images', 'news', or 'shopping'. Default is 'search'."
    )
    num_results: int = Field(10, description="Number of results to return (max 100)")
    location: Optional[str] = Field(None, description="Location for localized results (e.g., 'New York, NY')")
...
class SerpSearchTool:
    ...
    def run(
        ...
        if not self.api_key:
            return {"error": "SERP_API_KEY not found. Please set it in your environment variables."}
