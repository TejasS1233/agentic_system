"""API Registry for Toolsmith - provides access to free, no-auth public APIs."""
...
class APIRegistry:
    ...
    def __init__(
        self, registry_path: Optional[Path] = None, urls_path: Optional[Path] = None
    ):
    ...
    def _load_registry(self):
        ...
        try:
            ...
            logger.warning(f"Could not load API registry: {e}")
            self._apis = []
    ...
    def _load_urls(self):
        ...
        try:
            ...
            logger.warning(f"Could not load scrapable URLs: {e}")
            self._urls = []
    ...
    def search(self, query: str, limit: int = 5) -> List[Dict]:
        ...
        for api in self._apis:
            ...
            for terms in self.KEYWORD_MAP.values():
    ...
    def search_urls(self, query: str, limit: int = 3) -> List[Dict]:
        ...
        for url_entry in self._urls:
            ...
            for terms in self.SCRAPE_KEYWORD_MAP.values():
                if any(t in query_lower for t in terms):
    ...
    def format_for_prompt(self, apis: List[Dict]) -> str:
        ...
        for api in apis:
            ...
            lines.append(f"- **{name}**: `{method} {endpoint}` -> returns {returns}")
    ...
    def format_urls_for_prompt(self, urls: List[Dict]) -> str:
        ...
        for url_entry in urls:
            ...
            if selectors:
                selector_str = ", ".join(
                    [f"{k}: '{v}'" for k, v in list(selectors.items())[:2]]
                )
...
def format_all_sources_for_prompt(
    query: str, api_limit: int = 4, url_limit: int = 2
) -> str:
...
def main():
    ...
    for query in test_queries:
        ...
        logger.info(format_all_sources_for_prompt(query, api_limit=2, url_limit=2))
