    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "memory_used_percent": round(
                ...
                if self.gpu_memory_total_mb > 0 else 0, 2
            ),
    ...
    def _stop_profiling(self, success: bool = True, error_message: str = "") -> None:
        ...
        if tracemalloc.is_tracing():
            ...
            if self._memory_snapshot_start:
                ...
                self._result.memory_start_mb = max(0, self._result.peak_memory_mb - 10)  # Estimate
            ...
            if self._io_counters_start:
                try:
                    ...
                    self._result.io_metrics = IOMetrics(
                        disk_read_bytes=io_end.read_bytes - self._io_counters_start.read_bytes,
                        disk_write_bytes=io_end.write_bytes - self._io_counters_start.write_bytes,
                        disk_read_count=io_end.read_count - self._io_counters_start.read_count,
                        disk_write_count=io_end.write_count - self._io_counters_start.write_count,
                    )
                except (AttributeError, psutil.Error):
                    pass
...
class Profiler:
    ...
    def profile(
        ...
        **kwargs
        ...
        profile = ctx.result
        profile.return_value_type = type(result).__name__ if result is not None else "None"
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "success_rate": sum(1 for p in self._history if p.success) / len(self._history),
            }
    ...
    def get_tool_statistics(self, tool_name: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "success_rate": sum(1 for p in tool_profiles if p.success) / len(tool_profiles),
