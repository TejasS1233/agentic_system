...
for log_file in sorted(LOGS_DIR.glob("IASCIS_*.log")):
    ...
    all_steps_done = (num_steps is not None and steps_completed >= num_steps) or (num_steps is None and steps_completed > 0)
...
for r in successful:
    ...
    if q_key not in seen_queries or (r["total_time_ms"] and r["total_time_ms"] < (seen_queries[q_key]["total_time_ms"] or float("inf"))):
        seen_queries[q_key] = r

unique_successful = sorted(seen_queries.values(), key=lambda x: x["timestamp"])

print(f"\n{'='*80}")
print(f"  IASCIS - SUCCESSFUL TASK RUNS COMPILATION")
print(f"{'='*80}")
...
for i, r in enumerate(unique_successful, 1):
    ...
    print(f"      | Time: {total_s}s | Steps: {r['num_steps']} | Date: {r['timestamp']}")
    print(f"      | Tools: {tools_str}")
    
    if r["tool_profiles"]:
        for tp in r["tool_profiles"]:
            grade_icon = {"fast": "++", "moderate": "+ ", "slow": "- ", "critical": "--"}.get(tp["grade"], "??")
            print(f"      |   [{grade_icon}] {tp['tool']}: {tp['avg_time_ms']}ms, {tp['peak_mem_mb']}MB peak ({tp['grade']})")
...
output = {
    ...
    "aggregate": {
        "avg_time_s": round(sum(all_times)/len(all_times)/1000, 2) if all_times else 0,
...
print(f"  JSON summary saved to logs/successful_runs_summary.json")
